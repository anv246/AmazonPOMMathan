...
public class LoginPage {
	...
	By welcome_user = By.xpath("//a[@id='nav-link-yourAccount']/span[@class='nav-line-1']");
	By Sign_Out = By.xpath("//a[@id='nav-item-signout-sa']/*[contains(text(),'Sign Out')]");
	...
	public void loginValidation() throws Exception{
		...
		oWrapper.waituntilVisible(continueBtn,10);
		boolean con_Button = oWrapper.verifyElementpresent(continueBtn);
		if(con_Button==true){
			logger.log(LogStatus.PASS,"Login Button is presence in Amazon Home Page");
			oWrapper.clickElement(continueBtn);
		}else{
		...
		if(password_Textbox==true){
			logger.log(LogStatus.PASS,"Textbox is presence in Amazon Home Page");
		...
		if(sText.contains("Vijay")){
			...
			logger.attachScreenshot("D:\\Sample\\Amazon_POM\\Screenshot\\Signin_Failure.png", "Signin Failed");
		}
		...
		oWrapper.waitforPageLoad(30);
		login_Button = oWrapper.verifyElementpresent(emailLogin);
		if(login_Button==true){
